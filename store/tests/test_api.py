...
class BooksApiTestCase(APITestCase):
    def setUp(self) -> None:
        ...
        self.book_2 = Book.objects.create(name='Test Book 2', price=255, author_name='Author 5')
        self.book_3 = Book.objects.create(name='Test Book Author 1', price=255, author_name='Author 2')
    ...
    def test_get_filter(self):
        ...
        books = Book.objects.filter(id__in=[self.book_2.id, self.book_3.id]).annotate(
            annotated_likes=Count(Case(When(userbookrelation__like=True, then=1)))).order_by('id')
        response = self.client.get(url, data={'price': 255})
        serializer_data = BooksSerializer(books, many=True).data
        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(serializer_data, response.data)
